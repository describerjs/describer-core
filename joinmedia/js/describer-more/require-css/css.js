...
define(function() {
  ...
  var linkLoad = function(url, callback) {
    ...
    var linkUrl = url;
        ...
        _loadJM(linkElem, callback, function(){delayedAjaxCallForSaveInLocalStorage(url, linkUrl);});
        ...
        linkElem = document.createElement('link');
        linkElem.setAttribute('rel', 'stylesheet');
        _loadJM(linkElem, callback);
        linkElem.setAttribute('href', linkUrl);
        document.getElementsByTagName('head')[0].appendChild(linkElem);
    }
  };

  var delayedAjaxCallForSaveInLocalStorage = function(url, linkUrl){
    setTimeout(function(){
          var xhr;
  ...
  var _loadJM = function(link, callback, callbackForSaveInLocalStorage){
    if (useOnload){
      link.onload = function() {
        link.onload = function() {};
        // for style dimensions queries, a short delay can still be necessary
        setTimeout(callback, 7);
        if(typeof callbackForSaveInLocalStorage !== 'undefined'){
          setTimeout(callbackForSaveInLocalStorage, 7);
        }
      }
    }else{
      var loadInterval = setInterval(function() {

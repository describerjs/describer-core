	...
	var videoplayer = $.extend({}, _super, {
		...
		addHtml: function(){
			...
			/*this.$elem.prepend('<video class="video-js vjs-default-skin" controls preload="auto" poster="'+this.is('url')+'.jpg'+randomVal+'">'+
				...
				'</video>');*/
			this.$elem.prepend('<video class="video-js vjs-default-skin" width="'+ (this.is('width') * scalefactor)+'px" height="'+(this.is('height') * scalefactor) +'px" '+ this.is('loop') + ' ' + this.is('controls')+' preload="'+this.is('preload')+'" poster="'+this.is('url')+'.jpg'+randomVal+'">'+
				'<source src="'+this.is('url')+'.mp4" type="video/mp4" />'+
				'<source src="'+this.is('url')+'.webm" type="video/webm" />'+
				'<source src="'+this.is('url')+'.ogv" type="video/ogg" />'+
				'</video>');
		...
		addHtmlForYoutube: function(){
			...
			require(['videojs-youtube'], function () {
				...
				that.$elem.prepend('<video src="" id="'+ that.ID +'" class="video-js vjs-default-skin" width="'+ (that.is('width') * scalefactor)+'px" height="'+(that.is('height') * scalefactor) +'px" '+ that.is('loop') + ' ' + that.is('controls')+' preload="'+that.is('preload')+'"></video>');

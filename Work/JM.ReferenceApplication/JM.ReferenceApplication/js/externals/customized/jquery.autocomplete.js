define(['jquery'], function(jQuery){
	jQuery.autocomplete = function(input, options){
		...
		var jQueryinputUntouched = jQueryinput.siblings('.userinput');
		var jQueryinputbehind = jQueryinput.siblings('.behind');
		...
		if(options.width > 0){
			jQueryresults.css("width", options.width);
		}

		// Add to body element
		// Change by Joinmedia Begin
		if(options.jmappendToClass){
			jQuery('.' + options.jmappendToClass).eq(0).append(results);
		}else{
			jQuery("body").append(results);
		}
		// Change by Joinmedia End
		...
		jQueryinput.on('click touchstart', function(e){
			if(jQueryinputbehind.val().length === 0){
				return;
			}
			//selectCurrent()
			if(selectCurrent()){
		...
		jQueryinput.keydown(function(e){
				...
				switch(e.keyCode){
					...
					default:
						...
						setTimeout(function(){
							if(jQueryinput.val().length > jQueryinputUntouched.val().length){
								jQueryinputUntouched.val(jQueryinputbehind.val().substring(0,jQueryinput.val().length))
							}
							if(jQueryinputbehind.val() !== '' && jQueryinputbehind.val().substring(0,jQueryinput.val().length) !== jQueryinput.val()){
								jQueryinputbehind.val('');
							}
						}, 0);
						break;
				}
		...
		function moveSelect(step){
			...
			_text = jQuery(lis[active]).text();
			jQueryinputbehind.val(_text);

			jQueryinput.val(_text.substring(0,jQueryinput.val().length));
		...
		function showResults(){
			...
			var _cssObj = {
				...
				left : pos.x + options.offsetLeft + "px"
			};
			if(options.jmzindex){
				$.extend(true, _cssObj, { 'z-index': options.jmzindex });
			}
			jQueryresults.css(_cssObj).show();
			moveSelect(1);
			// Change by Joinmedia End
		}
		...
		function hideResultsNow(){
			...
			jQuery("li", results).removeClass('ac_over');
			jQueryinput.siblings('[disabled="disabled"]').val('');
		}
		...
		function receiveData(q, data){
			...
			////////////////////////////////////// Added By Joinmedia Begin ///////////////////////////////////
			if(options.jmhideByResult !== false){
				if(data.length === options.jmhideByResult){
					if(jQueryresults.is(":visible")){
						jQueryresults.hide();
					}
				}
			}
			////////////////////////////////////// Added By Joinmedia End ///////////////////////////////////
		}

		function parseData(data){
			if(!data) return null;
			var parsed = [];
			var rows = data.toString().split(options.lineSeparator);
			for(var i = 0; i < rows.length; i++){
				var row = jQuery.trim(rows[i]);
				if(row){
					parsed = row.toString().split(options.cellSeparator);
				}
			}
			return parsed;
		}
		...
		function requestData(q){
			...
			}else if((typeof options.url == "string") && (options.url.length > 0) && (q.length >= options.minChars)){
		...
		function makeUrl(q){
			...
			if(options.zipCodeBool){
				url += "&zipCode=" + jQueryinput.data('modules.formcomponents.autocomplete').getZipCode();
			}
	...
	jQuery.fn.autocomplete = function(url, options, data){
		...
		options.cacheLength = options.cacheLength || null;
		...
		options.submitForm = options.submitForm || false;

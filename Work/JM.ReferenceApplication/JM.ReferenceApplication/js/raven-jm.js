 ...
 *     }
	...
	TraceKit.computeStackTrace = (function computeStackTraceWrapper() {
		...
		function computeStackTraceFromStackProp(ex) {
			...
			var chrome = /^\s*at (?:((?:\[object object\])?\S+(?: \[as \S+\])?) )?\(?((?:file|https?|<anonymous>|chrome-extension):.*?):(\d+)(?::(\d+))?\)?\s*$/i,
			    gecko = /^\s*(\S*)(?:\((.*?)\))?@((?:file|https?|chrome).*?):(\d+)(?::(\d+))?\s*$/i,
			    ie10AndUp = /^\s*at (?:((?:\[object object\])?\S+(?: \[as \S+\])?) )?\(?((?:Unknown script code)):(\d+)(?::(\d+))?\)?\s*$/i,
			...
			for (var i = 0, j = lines.length; i < j; ++i) {
				...
				} else if ((parts = ie10AndUp.exec(lines[i]))) {
					element = {
						'url': parts[2],
						'func': parts[1] || UNKNOWN_FUNCTION,
						'line': +parts[3],
	...
	var Raven = {
		...
		config: function(dsn, options) {
			...
			// by JM globalKey = uri.user;
			// by JM globalProject = uri.path.substr(lastSlash + 1);

			// assemble the endpoint from the uri pieces
			/*// by JM globalServer = '//' + uri.host +
				(uri.port ? ':' + uri.port : '') +
	...
	function makeRequest(data) {
		var img = new Image(),
		    ...
		    src = globalServer + '&jm_data=' + encodeURIComponent(JSON.stringify(data));

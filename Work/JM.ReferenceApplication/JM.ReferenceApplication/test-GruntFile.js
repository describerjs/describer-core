...
module.exports = function(grunt){
		...
		return {
			...
			to: function(matchedWord, index, fullText, regexMatches){
				...
				function callback(abspath, rootdir, subdir, filename) {
					var str = filename;
					var regexpart = matchedWord.replace(/\'/g, "").replace(path, "");
					var regexpartWithoutVersionnumber = regexpart.substr(0, regexpart.length-6)+ '\\w{6}';
					grunt.log.writeln([str]);
					var patt = new RegExp('^'+regexpartWithoutVersionnumber+'\\.(.*)');
					grunt.log.writeln([patt]);
	...
	grunt.registerTask('task_step2', function(){
		grunt.initConfig({
			replace: {
				example: {
					src: ['Views/Shared/MainJS.cshtml'],             // source files array (supports minimatch)
					dest: 'Views/Shared/MainJS.cshtml',             // destination directory or file
					replacements: [
